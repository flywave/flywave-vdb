...
# Usage samples:
#   ss_assert(MyLib_FOUND)
#   ss_assert(DEFINED MyLib_INCLUDE_DIRS)
macro(ss_assert)
  if(NOT (${ARGN}))
...
macro(ss_check_environment_variables)
  foreach(_var ${ARGN})
...
function(ss_include_directories)
  set(__add_before "")
  foreach(dir ${ARGN})
...
MACRO(ss_check_compiler_flag LANG FLAG RESULT)
  if(NOT DEFINED ${RESULT})
    ...
    if(_fname)
      ...
      FOREACH(_regex ${SS_COMPILER_FAIL_REGEX})
...
macro(ss_check_flag_support lang flag varname)
  if("_${lang}_" MATCHES "_CXX_")
    set(_lang CXX)
  elseif("_${lang}_" MATCHES "_C_")
    set(_lang C)
  ...
  ss_check_compiler_flag("${_lang}" "${ARGN} ${flag}" ${${varname}})
endmacro()

# turns off warnings
macro(ss_warnings_disable)
    ...
    elseif((CMAKE_COMPILER_IS_GNUCXX OR (UNIX AND CV_ICC)) AND _gxx_warnings AND _flag_vars)
      foreach(var ${_flag_vars})
        foreach(warning ${_gxx_warnings})
          ...
          ss_check_flag_support(${var} "${warning}" _varname)
          if(${_varname})
            set(${var} "${${var}} ${warning}")
          endif()
        endforeach()
...
macro(SS_OPTION variable description value)
  set(__value ${value})
  set(__condition "")
  ...
  if(DEFINED status_cond)
    ...
    if(DEFINED status_then OR DEFINED status_else)
      if(${status_cond})
        ...
        ss_output_status("${status_text} ${status_then}")
      else()
        string(REPLACE ";" " " status_else "${status_else}")
        string(REGEX REPLACE "^[ \t]+" "" status_else "${status_else}")
        ss_output_status("${status_text} ${status_else}")
      endif()
    else()
      string(REPLACE ";" " " status_cond "${status_cond}")
      string(REGEX REPLACE "^[ \t]+" "" status_cond "${status_cond}")
      ss_output_status("${status_text} ${status_cond}")
    endif()
  else()
    ss_output_status("${text}")
  endif()
...
macro(ss_split_libs_list lst lstdbg lstopt)
  set(${lstdbg} "")
  set(${lstopt} "")
  set(perv_keyword "")
  foreach(word ${${lst}})
...
macro(ss_list_filterout lst regex)
  foreach(item ${${lst}})
...
macro(ss_list_add_prefix LST PREFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(ss_list_add_suffix LST SUFFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(ss_list_pop_front LST VAR)
  if(${LST})
    list(GET ${LST} 0 ${VAR})
...
macro(ss_regex_escape var regex)
  string(REGEX REPLACE "([+.*^$])" "\\\\1" ${var} "${regex}")
endmacro()


# convert list of paths to full paths
macro(ss_convert_to_full_paths VAR)
  if(${VAR})
    set(__tmp "")
    foreach(path ${${VAR}})
...
function(ss_convert_to_lib_name var)
  set(tmp "")
  foreach(path ${ARGN})
...
function(ss_create_imported_targets var)
  set(target_list "")

  foreach(library ${ARGN})
    ss_convert_to_lib_name(libname "${library}")
...
macro(ss_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
macro(ss_parse_header2 LIBNAME HDR_PATH VARNAME)
  ss_clear_vars(${LIBNAME}_VERSION_MAJOR
...
macro(ss_parse_pkg LIBNAME PKG_PATH SCOPE)
...
function(ss_source_group group)
  cmake_parse_arguments(SS_SOURCE_GROUP "" "" "GLOB" ${ARGN})
  file(GLOB srcs ${SS_SOURCE_GROUP_GLOB})
...
macro(ss_extract_simple_libs _all_libs _simple _other)
  set(${_simple} "")
  set(${_other} "")
