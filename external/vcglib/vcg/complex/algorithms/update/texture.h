...
static int WedgeTexMergeClose(ComputeMeshType &m, ScalarType mergeThr = ScalarType(1.0/65536.0) )
{
  ...
  ForEachVertex(m, [&](VertexType &v){
    ...
    std::vector<UVCoordType> clusterVec;
      clusterVec.push_back(UVCoordType{vfi.F()->WT(vfi.I()).P()[0], vfi.F()->WT(vfi.I()).P()[1]});
    ++vfi;
    while(!vfi.End())
    {
      UVCoordType cur= UVCoordType{vfi.F()->WT(vfi.I()).P()[0], vfi.F()->WT(vfi.I()).P()[1]};
      bool merged=false;
      for(auto p:clusterVec) {
        if(p!=cur && Distance(p,cur) < mergeThr){ 
          vfi.F()->WT(vfi.I()).P()[0]=p[0];
          vfi.F()->WT(vfi.I()).P()[1]=p[1];
          ++mergedCnt;
          merged=true;
        }
      }
