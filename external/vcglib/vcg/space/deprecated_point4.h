...
public:
	...
	enum {Dimension = 4};
    template <class OScalarType> friend class Point4;
...
//@{
   ...
   **/
    ...
    inline Point4 & operator =(Point4<OScalarType> const & p)
    {
      _v[0] = static_cast<T>(p._v[0]); _v[1] = static_cast<T>(p._v[1]); _v[2] = static_cast<T>(p._v[2]);
      _v[3] = static_cast<T>(p._v[3]);
      return *this;
    }
	inline void SetZero()
	{
