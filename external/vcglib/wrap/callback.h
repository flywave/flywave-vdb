...
#include <iostream> // For std::unique_ptr
 ...
 * reporting the what the algorithm is doing and at what point of the processing
 * (as a int percentage) we currently are.
 *
 * Users of the library usually rewrote this function to handle this reporting
 * in the correct way (e.g. in a log window and in a progress bar).
 *
 * The algorithms in the library should also check the return value
 * to see if the user asked for an interruption, but they can ignore this
 * request (hint: most of algs in the lib ignore any request).
 */
...
/*! \brief Helper function that formats a string a la printf and returns a
 * simple char*
 *
