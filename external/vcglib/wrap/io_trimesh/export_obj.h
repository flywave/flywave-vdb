...
template <class SaveMeshType> class ExporterOBJ {
public:
  ...
  /*
            this function takes an index and the relative error message gets
     back
        */
  static const char *ErrorMsg(int error) {
    ...
    if (error > 9 || error < 0)
      ...
      return obj_error_msg[error];
  };

  /*
            returns mask of capability one define with what are the saveable
   ...
   *
   * if you enable the useMaterialAttribute flag, the exporter will assume that
   * the mesh has a consistent per mesh attribute and a per face attribute
   * containing the index of the material
   */
  static int Save(SaveMeshType &m, const char *filename, int mask,
                  bool useMaterialAttribute, CallBackPos *cb = 0) {
    // texture coord and color: in obj we cannot save BOTH per vertex and per
    // wedge information. We default on wedge
    ...
    if (m.vn == 0)
      return E_NO_VERTICES;

    typename SaveMeshType::template PerMeshAttributeHandle<
        std::vector<Material>>
        materialVecHandle =
            vcg::tri::Allocator<SaveMeshType>::template FindPerMeshAttribute<
                std::vector<Material>>(m, "materialVector");
    typename SaveMeshType::template PerFaceAttributeHandle<int>
        materialIndexHandle =
            vcg::tri::Allocator<SaveMeshType>::template FindPerFaceAttribute<
                int>(m, "materialIndex");

    if (useMaterialAttribute &&
        (!Allocator<SaveMeshType>::IsValidHandle(m, materialVecHandle)) &&
        (!Allocator<SaveMeshType>::IsValidHandle(m, materialIndexHandle)))
      return E_NO_VALID_MATERIAL;

    FILE *fp = fopen(filename, "w");
    if (fp == NULL)
      return E_CANTOPENFILE;
    ...
    if ((mask & vcg::tri::io::Mask::IOM_FACECOLOR) ||
        (mask & Mask::IOM_WEDGTEXCOORD) || (mask & Mask::IOM_VERTTEXCOORD))
    ...
    for (auto vi = m.vert.begin(); vi != m.vert.end(); ++vi)
      if (!(*vi).IsD()) {
        ...
        if (mask & Mask::IOM_VERTCOLOR) // the socially accepted extension to
                                        // the obj format.
          fprintf(fp, " %f %f %f", double((*vi).C()[0]) / 255.,
                  double((*vi).C()[1]) / 255., double((*vi).C()[2]) / 255.);
        fprintf(fp, "\n");

        if (cb != NULL) {
          if (!(*cb)((100 * ++current) / totalPrimitives,
                     "writing vertices ")) {
            fclose(fp);
            return E_ABORTED;
          }
    ...
    int curMatIndex = -1;
    std::vector<Material>
        materialVec; // used if we do not have material attributes
    ...
    for (FaceIterator fi = m.face.begin(); fi != m.face.end(); ++fi)
      if (!(*fi).IsD()) {
        if ((mask & Mask::IOM_FACECOLOR) || (mask & Mask::IOM_WEDGTEXCOORD) ||
          ...
          int index = -1;
          if (useMaterialAttribute)
            index = materialIndexHandle[fi];
          else
            index =
                Materials<SaveMeshType>::CreateNewMaterial(m, materialVec, fi);
        ...
        if (HasPerWedgeTexCoord(m) && (mask & Mask::IOM_WEDGTEXCOORD))
          for (int k = 0; k < (*fi).VN(); k++) {
            if (AddNewTextureCoord(CoordIndexTexture, (*fi).WT(k),
                                   curTexCoordIndex)) {
              fprintf(fp, "vt %f %f\n", (*fi).WT(k).u(), (*fi).WT(k).v());
              curTexCoordIndex++; // ncreases the value number to be associated
                                  // to the Texture
            }
          }
        ...
        for (int k = 0; k < (*fi).VN(); k++) {
          if (k != 0)
            fprintf(fp, " ");
          int vInd = -1;
          // +1 because Obj file format begins from index = 1 but not from index
          // = 0.
          vInd = VertexId[tri::Index(m, (*fi).V(k))] +
                 1; // index of vertex per face

          int vt = -1;
          if (mask & Mask::IOM_WEDGTEXCOORD)
            vt = GetIndexVertexTexture(
                CoordIndexTexture,
                (*fi).WT(k)); // index of vertex texture per face
          if (mask & Mask::IOM_VERTTEXCOORD)
            vt = vInd;

          int vn = -1;
          if (mask & Mask::IOM_WEDGNORMAL)
            vn = GetIndexVertexNormal(
                m, NormalVertex,
                (*fi).V(k)->cN()); // index of vertex normal per face.
          if (mask & Mask::IOM_VERTNORMAL)
            vn = vInd;
    ...
    if ((mask & Mask::IOM_WEDGTEXCOORD) || (mask & Mask::IOM_FACECOLOR) ||
        (mask & Mask::IOM_VERTTEXCOORD)) {
      if (useMaterialAttribute)
        errCode = WriteMaterials(materialVecHandle(), filename, cb);
      else
        errCode = WriteMaterials(materialVec, filename, cb);
    }
  ...
  /*
       ...
       */
  inline static int
  GetIndexVertexTexture(typename std::map<TexCoordType, int> &mapTexToInt,
                        const TexCoordType &wt) {
    typename std::map<TexCoordType, int>::iterator iter = mapTexToInt.find(wt);
    if (iter != mapTexToInt.end())
      return (*iter).second;
    else
      return -1;
    // Old wrong version.
  ...
  /*
        ...
        */
  inline static int GetIndexVertexNormal(SaveMeshType & /*m*/,
  ...
  /*
            adds a new index to the coordinate of Texture if it is the first
     time which is otherwise met not execute anything
        */
  template <class TexScalarType>
  inline static bool
  AddNewTextureCoord(std::map<typename vcg::TexCoord2<TexScalarType>, int> &m,
  ...
  /*
        ...
        */
  inline static bool AddNewNormalVertex(typename std::map<CoordType, int> &m,
                                        CoordType &n, int value) {
    int index = m[n];
    if (index == 0) {
      m[n] = value;
      return true;
  ...
  /*
        ...
        */
  inline static int WriteMaterials(std::vector<Material> &materialVec,
    ...
    if (materialVec.size() > 0) {
      ...
      for (unsigned int i = 0; i < materialVec.size(); i++) {
        if (cb != NULL)
          (*cb)((100 * ++current) / materialVec.size(),
                "saving material file ");
        else { /* fclose(fp); return E_ABORTED; */
        }

        fprintf(fp, "newmtl material_%d\n", i);
        fprintf(fp, "Ka %f %f %f\n", materialVec[i].Ka[0], materialVec[i].Ka[1],
                materialVec[i].Ka[2]);
        fprintf(fp, "Kd %f %f %f\n", materialVec[i].Kd[0], materialVec[i].Kd[1],
                materialVec[i].Kd[2]);
        fprintf(fp, "Ks %f %f %f\n", materialVec[i].Ks[0], materialVec[i].Ks[1],
                materialVec[i].Ks[2]);
        fprintf(fp, "Tr %f\n", materialVec[i].Tr);
        fprintf(fp, "illum %d\n", materialVec[i].illum);
        fprintf(fp, "Ns %f\n", materialVec[i].Ns);
