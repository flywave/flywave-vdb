...
Improved the compatibility for ply format for faces having the list size (e.g.
number of vertexes of a face) as a char instead of a uchar. Added a couple of
new face descriptors, corrected a bug in error reporting function (and
restructured) and translated a few comments. Thanks to Patrick Min for the
...
public:
  ...
  int stotype1;   // Tipo dell'elemento su file    (se lista tipo degli elementi
                  // della lista)
  int memtype1;   // Tipo dell'elemento in memoria (se lista tipo degli elementi
                  // della lista)
  ...
  size_t stotypesize()
      const; // per sapere quanto e'grande un dato descrittore sul file
  size_t memtypesize()
...
public:
  ...
  inline void SetName(const char *na) { name = std::string(na); }
  ...
  int AddToRead(const char *propname, int stotype1, int memtype1,
                size_t offset1, int islist, int alloclist, int stotype2,
                int memtype2,
                size_t offset2); // Vedi struttura PropDescriptor
...
public:
  ...
  int AddToRead(const char *elemname, const char *propname, int stotype1,
                int memtype1, size_t offset1, int islist, int alloclist,
                int stotype2, int memtype2, size_t offset2);
  // Come sopra ma con descrittore
  inline int AddToRead(const PropDescriptor &p) {
    return AddToRead(p.elemname, p.propname, p.stotype1, p.memtype1, p.offset1,
                     p.islist, p.alloclist, p.stotype2, p.memtype2, p.offset2);
  }
