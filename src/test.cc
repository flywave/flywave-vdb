...
extern "C" {
#endif
_Bool borderLockCheck(void *ctx, float *a) {
    return false;
}
...
void build_quad(std::shared_ptr<mesh_data> md, openvdb::Vec3s offset, openvdb::Vec3s widthDir, openvdb::Vec3s lengthDir) {
    ...
    md->add_mtl_normal(-1, openvdb::Vec3I(baseIndex, baseIndex+1, baseIndex+2));
    md->add_mtl_texcoord(-1, openvdb::Vec3I(baseIndex, baseIndex+1, baseIndex+2));

    md->add_mtl_face(-1, openvdb::Vec3I(baseIndex, baseIndex+2, baseIndex+3));
    md->add_mtl_normal(-1, openvdb::Vec3I(baseIndex, baseIndex+2, baseIndex+3));
    md->add_mtl_texcoord(-1, openvdb::Vec3I(baseIndex, baseIndex+2, baseIndex+3));
}
...
public:
  bool operator()(vertex_grid::Ptr vertex,
...
int test_mesh_builder() {
    std::shared_ptr<flywave::voxel_mesh_builder> vmb = std::make_shared<flywave::voxel_mesh_builder>();
    ...
    std::shared_ptr<flywave::voxel_pixel> stff_pot =
        sampler.apply(10, createor, sampler_type::level_set,
                      tmtl, openvdb::Mat4d::identity());
