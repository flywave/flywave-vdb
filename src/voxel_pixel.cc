...
bool voxel_pixel::is_sdf() const {
  if (_vertex->getGridClass() == openvdb::GRID_LEVEL_SET)
    return true;
...
double voxel_pixel::get_volume() const {
  double volume = 0;
  vdb_calc_volume(*_vertex, volume);
  return volume;
}

double voxel_pixel::get_area() const {
  double area = 0;
  vdb_calc_area(*_vertex, area);
  return area;
}

namespace {

template <bool NORMALIZE> class vdb_eval_gradients {
public:
  vdb_eval_gradients(openvdb::Vec3d *gradients, int stride,
                     const openvdb::Vec3d *positions, int num_positions)
      : myGradients(gradients), myStride(stride), myPos(positions),
  ...
  template <typename GridT> void operator()(const GridT &grid) {
    ...
    const double vox_size = std::min(dim[0], std::min(dim[1], dim[2]));
    const double h = 0.5 * vox_size;
    const math::Vec3d mask[] = {math::Vec3d(h, 0, 0), math::Vec3d(0, h, 0),
                                math::Vec3d(0, 0, h)};
    AccessorT accessor = grid.getConstAccessor();
    openvdb::Vec3d *gradient = myGradients;
...
private:
  openvdb::Vec3d *myGradients;
  int myStride;
  const openvdb::Vec3d *myPos;
  int myNumPos;
};

} // namespace

bool voxel_pixel::eval_gradients(openvdb::Vec3d *gradients, int stride,
                                 const openvdb::Vec3d *pos, int num_pos,
                                 bool normalize) const {
  if (normalize) {
    vdb_eval_gradients<true> eval(gradients, stride, pos, num_pos);
    eval(*_vertex);
  } else {
    vdb_eval_gradients<false> eval(gradients, stride, pos, num_pos);
    eval(*_vertex);
  }
  return true;
}

openvdb::Vec3d voxel_pixel::get_gradient(const openvdb::Vec3d &pos) const {
  openvdb::Vec3d grad;

  grad.setZero();

  eval_gradients(&grad, 1, &pos, 1, false);

  return grad;
}

double voxel_pixel::calc_positive_density() const {
  double density = 0;

  vdb_sum_pos_density(*_vertex, density);

  int numvoxel = _vertex->activeVoxelCount();
  if (numvoxel)
    density /= numvoxel;
