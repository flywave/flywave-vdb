...
std::vector<texture_atlas_generate::texture_ptr> &
texture_atlas_generate::generate(simplify_mesh &_texture_mesh,
                                 simplify_mesh &output_mesh) {
  int current_texture_id = -1;
  _textures.clear();
  std::unordered_map<int, std::vector<simplify_face *>> texture_index_map;
  std::vector<simplify_face *> *_count_ptr = nullptr;
  ...
  std::unordered_map<int, simplify_mesh> texture_meshies;

  for (auto &iter : texture_index_map) {
    vcg::tri::Allocator<simplify_mesh>::AddFaces(texture_meshies[iter.first],
                                                 iter.second.size());
    vcg::tri::Allocator<simplify_mesh>::AddVertices(texture_meshies[iter.first],
                                                    iter.second.size() * 3);
    int face_index = 0;
    ...
    vcg::tri::Clean<simplify_mesh>::RemoveDuplicateVertex(
        texture_meshies[iter.first]);
    vcg::tri::Allocator<simplify_mesh>::CompactVertexVector(
        texture_meshies[iter.first]);
    vcg::tri::Allocator<simplify_mesh>::CompactFaceVector(
        texture_meshies[iter.first]);
    vcg::tri::UpdateNormal<simplify_mesh>::PerVertex(
        texture_meshies[iter.first]);
  }

  auto sum_face_size = 0;
  for (auto &iter : texture_index_map) {
    auto packed = std::make_shared<packed_mesh<xparam<simplify_mesh>>>(
  ...
  {
    output_mesh.Clear();
    vcg::tri::Allocator<simplify_mesh>::AddFaces(output_mesh, sum_face_size);
    vcg::tri::Allocator<simplify_mesh>::AddVertices(output_mesh,
                                                    sum_face_size * 3);
    ...
    vcg::tri::Clean<simplify_mesh>::RemoveDuplicateVertex(output_mesh);
    vcg::tri::Allocator<simplify_mesh>::CompactVertexVector(output_mesh);
    vcg::tri::Allocator<simplify_mesh>::CompactFaceVector(output_mesh);
    vcg::tri::UpdateNormal<simplify_mesh>::PerVertex(output_mesh);
  }

  return _textures;
}

void texture_atlas_generate::push_foundry_atlas(
