...
public:
  ...
  typedef openvdb::math::Vec3<value_type_t> vector_type_t;

  openvdb::math::Vec3<scalar_type> normal;
  scalar_type distance;

  plan() noexcept : normal(), distance() {}
  plan(const plan &r) noexcept : normal(r.normal), distance(r.distance) {}
  plan(const openvdb::math::Vec3<scalar_type> &normal,
       const scalar_type &distance) noexcept
      : normal(normal), distance(distance) {}
  plan(const openvdb::math::Vec3<scalar_type> &normal,
       const openvdb::math::Vec3<scalar_type> &point) noexcept
      : normal(normal), distance(-(point).dot(normal)) {}
  plan(const openvdb::math::Vec3<scalar_type> &v0,
       const openvdb::math::Vec3<scalar_type> &v1,
       const openvdb::math::Vec3<scalar_type> &v2) noexcept
  ...
  scalar_type
  distance_to_point(const openvdb::math::Vec3<scalar_type> &point) const {
    return (point).dot(normal) + distance;
  }

  vector_type
  project_point(const openvdb::math::Vec3<scalar_type> &point) const {
    return normal * (-distance_to_point(point)) + point;
  }

  scalar_type dot(const openvdb::math::Vec3<scalar_type> &point) const {
    return normal.dot(point);
  }

  bool behind(const openvdb::math::Vec3<scalar_type> &point) const {
    return dot(point) < 0.f;
  }

  openvdb::math::Vec3<scalar_type> position() const {
    return normal * distance;
  }

  bool intersects(const openvdb::math::Ray<scalar_type> &ray) const {
    scalar_type t0 = openvdb::math::Delta<scalar_type>();
    scalar_type t1 = std::numeric_limits<scalar_type>::max();
    const scalar_type cosAngle = ray.direction.dot(normal);
    if (openvdb::math::isApproxEqual(cosAngle))
      return false;
