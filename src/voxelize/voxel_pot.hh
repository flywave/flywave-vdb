...
public:
  voxel_pot()
      : _vertex(vertex_grid::create(std::numeric_limits<float>::max())),
        ...
        _resolution(openvdb::math::Transform::createLinearTransform()) {
    _vertex->setTransform(_resolution);
    _vertex->setGridClass(openvdb::GRID_LEVEL_SET);
  }

  voxel_pot(openvdb::math::Transform::Ptr transfrom)
      : _vertex(vertex_grid::create(std::numeric_limits<float>::max())),
  ...
  openvdb::math::Transform::Ptr voxel_resolution() const { return _resolution; }
...
private:
  struct file_patch {
    openvdb::math::Transform::Ptr _resolution;
...
public:
  ...
  bool ray_test(const flywave::extray3<double> &ray, openvdb::Vec3d &p);
